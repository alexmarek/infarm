...
// assig function to marker info window
...
function sortLocationList(data) {
    bounds = new google.maps.LatLngBounds();
    for (var x in data.farms) {
        if (data.farms.hasOwnProperty(x)) {
            var myLatlng = new google.maps.LatLng(data.farms[x].coordinates.lat, data.farms[x].coordinates.lng);
            ...
            marker = new google.maps.Marker({
                ...
                position: myLatlng,
                map: map,
                organization: data.farms[x]['organization'],
                category: data.farms[x]['category'],
                postcode: data.farms[x]['postcode'],
                address: data.farms[x]['line_1'],
                country: data.farms[x]['country'],
                icon: '/wp-content/themes/infarm-child/images/map-marker-alt.svg'
            });
            if (data.farms[x]['line_1']){
              dataForInfoWindow = 
              `<span class="map-infopanel"><strong>${data.farms[x]['category']} </strong> 
              <h4>${data.farms[x]['organization']} </h4> 
              <p>${data.farms[x]['line_1']} <br> 
              ${data.farms[x]['postcode']}  
              ${data.farms[x]['country']}</p>
              <h5 class="map-directions" data-coordinates="${myLatlng}" onclick="showMeDirection(this)">Directions</h5></span>
              `;
            }
            else {
              dataForInfoWindow = 
            ...
            var loc = new google.maps.LatLng(data.farms[x].coordinates.lat, data.farms[x].coordinates.lng);
            bounds.extend(loc);
...
function getMyLocation(e) {
    ...
    function successFunction(position) {
        ...
        window.location.hash = 'location-getMylocation';
    }
...
// 
function updateMap(lat, lng, myInfoWindow) {
  window.location.hash = '';
...
function showNumberOfResults(value) {
    ...
    for (var i = 0; i < allmarkers.length; i++) {
      if (allmarkers[i].category === value) {
          ...
          listResultsContainer.innerHTML += `
            <div class="container-fluid info info-${i + 1}">
                <div class="row mt-3">
                    <div class="col-md-12"><span>${allmarkers[i].category}</span></div>
                    <div class="col-lg-3"><h4>${allmarkers[i].organization}</h4></div>
                    <div class="col-lg-7"><p>${allmarkers[i].address}, ${allmarkers[i].postcode} ${allmarkers[i].country}</p></div>
                    <div class="col-lg-2"><h5 data-coordinates="${allmarkers[i].position}" onclick="showMeDirection(this)">Directions</h5></div>
                </div>
            </div>
              `;
          infoPanel = jQuery('.info-' + (i + 1));
  ...
  if (!value) {
    for (var i = 0; i < allmarkers.length; i++) {
        ...
        listResultsContainer.innerHTML += `
          <div class="container-fluid info info-${i + 1}">
              <div class="row mt-3">
                  <div class="col-lg-12"><span>${allmarkers[i].category}</span></div>
                  <div class="col-lg-3"><h4>${allmarkers[i].organization}</h4></div>
                  <div class="col-lg-7"><p>${allmarkers[i].address}, ${allmarkers[i].postcode} ${allmarkers[i].country}</p></div>
                  <div class="col-lg-2"><h5 data-coordinates="${allmarkers[i].position}" onclick="showMeDirection(this)">Directions</h5></div>
              </div>
  ...
  document.getElementById('results-number').innerHTML = count;
}
